...
#VPATH              = Bitmaps
SOURCES            = $(wildcard *.c) $(wildcard */*.c) #$(wildcard bitmaps/*.c) #bitmaps\splashscreen.c
#SOURCES            = $(wildcard Bitmaps\*.c)
ASMSOURCES         = $(wildcard *.asm) $(wildcard */*.s)
CCPATH             = C:\cygwin64\bin
DKPATH             = C:\Users\marcus_samuel\Desktop\devkitPro
VBASIM             = C:\Users\marcus_samuel\Documents\College\CS2261\Programs\vbam-2.0.1\bin\visualboyadvance-m.exe
FIND               = find
COPY               = cp -r
...
CFLAGS             = $(MODEL) -IC -O2 -Wall -pedantic -Wextra -std=c99 -save-temps -D_ROM=$(ROM_NAME) -D_VBA=$(VBASIM)
